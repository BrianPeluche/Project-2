...
def backward_elimination(feat_num, feat_idx):
    curr_set = list(range(1, feat_num + 1))
    best_set = curr_set.copy()
    best_accuracy = feat_idx(curr_set)
    ...
    for level in range(feat_num, 0, -1):
        feature_remove_level = None
        best_accuracy_level = -1.0

        for feature in curr_set:
            temp_set = [f for f in curr_set if f != feature]
            accuracy = feat_idx(temp_set)

            feat_str = ",".join(str(f) for f in temp_set)
            print(f"Using feature(s) {{{feat_str}}} accuracy is {accuracy:.1f}%")

            if accuracy > best_accuracy_level:
                best_accuracy_level = accuracy
                feature_remove_level = feature

        if feature_remove_level is not None:
            curr_set.remove(feature_remove_level)

            if best_accuracy_level > best_accuracy:
                best_accuracy = best_accuracy_level
                best_set = curr_set.copy()
                features_str = ",".join(str(f) for f in curr_set)
                print(f"Feature set {{{features_str}}} was best, accuracy is {best_accuracy_level:.1f}%\n")
